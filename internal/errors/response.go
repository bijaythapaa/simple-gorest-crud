...
// InternalServerError creates a new error response
// representing an internal server error (HTTP 500)
...
func NotFound(msg string) ErrorResponse {
	if msg == "" {
		msg = "The requested resoure was not Found."
	}
	return ErrorResponse{
...
// Unauthorized creates a new error response representing
// an authentication/authorization failure (HTTP 401)
...
func InvalidInput(errs validation.Errors) ErrorResponse {
	...
	return ErrorResponse{
		Status:  http.StatusBadRequest,
		Message: "There some problems with the data you submitted.",
		Details: details,
	}
}
