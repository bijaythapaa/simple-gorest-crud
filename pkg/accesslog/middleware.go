// Package accessslog provides a middleware that records every RESTful API call in a log message.
package accesslog
...
func Handler(logger log.Logger) routing.Handler {
	return func(c *routing.Context) error {
		...
		// so that they can be added to the log message
		...
		logger.With(ctx, "duration", time.Now().Sub(start).Milliseconds(), "status", rw.Status).
			Infof("%s %s %s %d %d", c.Request.Method, c.Request.URL.Path, c.Request.Proto, rw.Status,
				rw.BytesWritten)
		return err
	}
}
