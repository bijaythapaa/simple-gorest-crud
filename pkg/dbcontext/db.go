...
// DB represents a database connection that can be used to run SQL queries.
type DB struct {
	db *dbx.DB
}

// TransactionFunc represents a function that will start a transaction
...
// Transactional starts a transaction and calls the given function
// with a context storing the transaction.
